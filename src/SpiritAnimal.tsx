...
export default function AnimalConstellationApp() {
  ...
  const IntroScreen = () => (
    <div className="max-w-xl mx-auto text-center pt-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="mb-8 flex justify-center">
        <div className="w-16 h-16 bg-slate-900 rounded-full flex items-center justify-center text-slate-100">
          <Activity size={32} strokeWidth={1.5} />
        </div>
      </div>
      <h1 className="text-4xl font-light text-slate-900 mb-6 tracking-tight">
      ...
      <button 
        ...
        className="group bg-slate-900 text-white px-8 py-4 rounded-lg font-medium hover:bg-slate-800 transition-all flex items-center gap-2 mx-auto"
      >
        Begin Mapping
  ...
  const TraitScreen = () => (
    <div className="max-w-2xl mx-auto pt-4 animate-in fade-in duration-500">
      <div className="mb-8">
        <h2 className="text-2xl font-light text-slate-900 mb-2">Trait Input</h2>
        <p className="text-slate-500 text-sm">Adjust sliders to match your typical patterns.</p>
      </div>
            ...
            <div key={t.id} className="space-y-3">
              <div className="flex justify-between items-end">
                ...
                <div className="text-right">
                  ...
                  <span className="font-mono text-slate-500 w-8 inline-block text-right">
                    {inputs[t.id]}
                  </span>
                </div>
              </div>
      ...
      <div className="mt-8 flex justify-center">
        <button 
          ...
          className="w-full md:w-auto bg-slate-900 text-white px-12 py-4 rounded-lg font-medium hover:bg-slate-800 transition-colors shadow-lg shadow-slate-200"
        >
  ...
  const ResultsScreen = () => (
    <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-700">
      <div className="flex justify-between items-start mb-8 border-b border-slate-200 pb-6">
        <div>
          <h2 className="text-3xl font-light text-slate-900 mb-2">Your Constellation</h2>
          <p className="text-slate-600">These strategies align with your trait bands.</p>
        </div>
        <button 
          onClick={() => setScreen('traits')}
        ...
        {results.map((animal, idx) => (
          <div 
            ...
            className={`
              ...
              ${idx === 0 ? 'bg-slate-50 border-slate-200 shadow-md' : 'bg-white border-slate-100 hover:border-slate-300'}
            `}
          >
            {/* Rank Badge */}
            <div className="absolute top-8 left-8 w-10 h-10 flex items-center justify-center rounded-full border border-slate-200 text-slate-400 font-mono text-sm">
              0{idx + 1}
            </div>

            <div className="pl-16">
              <div className="flex flex-col md:flex-row md:items-center gap-4 mb-4">
                <h3 className="text-2xl font-medium text-slate-900">{animal.name}</h3>
                <div className="flex gap-2">
                  {Object.entries(animal.traits).map(([k, v]) => (
                    <span key={k} className="text-[10px] uppercase tracking-widest bg-slate-200 text-slate-600 px-2 py-1 rounded-sm">
                      {k}:{v.replace('_', ' ')}
                    </span>
                  ))}
                </div>
              ...
              <p className="text-slate-700 text-lg mb-6 leading-relaxed border-l-2 border-slate-300 pl-4">
                {animal.description}
              </p>

              {/* Dynamic Contextual Explanation */}
              <div className="mb-6 text-sm text-slate-600 italic bg-white/50 p-3 rounded">
                "Aligns with your 
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Effective Strengths</h4>
                ...
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Potential Limitations</h4>
                  <ul className="space-y-2">
                    {animal.limitations.map(l => (
                      <li key={l} className="text-sm text-slate-600 flex items-start gap-2">
      ...
      <div className="mt-12 bg-slate-50 p-6 rounded-lg flex gap-4 items-start text-slate-500 text-sm leading-relaxed">
